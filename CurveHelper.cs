...
public class CurveHelper
{
    public static AnimCurveFrameType GetFrameType(uint FrameCount)
    {
        ...
        if (FrameCount < short.MaxValue) return AnimCurveFrameType.Decimal10x5;
        return AnimCurveFrameType.Single;
    }

    public static AnimCurveKeyType GetKeyType(float Value)
    {
        if (Value < byte.MaxValue) return AnimCurveKeyType.SByte;
        if (Value < short.MaxValue) return AnimCurveKeyType.Int16;
        return AnimCurveKeyType.Single;
    }

    public static Animation.KeyGroup CreateTrackWiiU(AnimCurve curve, bool valuesAsInts = false)
    {
        ...
        var valueScale = curve.Scale > 0 ? curve.Scale : 1;

        for (var i = 0; i < curve.Frames.Length; i++)
        {
    ...
    public static float[] GetSlopesNX(AnimCurve curve, float index)
    {
        var slopes = new float[2];
    ...
    public static float[] GetSlopesU(AnimCurve curve, float index)
    {
        var slopes = new float[2];
        if (curve.CurveType == AnimCurveType.Cubic)
        {
    ...
    {
        var outSlope = coef[1] / time;
        var param = coef[3] - -2 * delta;
        var inSlope = param / time - outSlope;
        return new float[2] { inSlope, coef[1] == 0 ? 0 : outSlope };
    }

    public static BooleanKeyGroup CreateBooleanTrackWiiU(AnimCurve animCurve)
    {
        ...
        for (var i = 0; i < (ushort)animCurve.Frames.Length; i++)
        ...
        for (var i = 0; i < curve.Frames.Length; i++)
        {
            ...
            switch (curve.CurveType)
            {
                case AnimCurveType.Cubic:
                {
                    ...
                    //Important to not offset the other 3 values, just the first one!
                    var value = curve.Keys[i, 0] * valueScale + curve.Offset;
                    var slopes = GetSlopesNX(curve, i);
